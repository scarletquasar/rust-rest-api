...
use crate::endpoints::response_helpers::{
    fecth_create_users_result, 
    fetch_create_user_result, 
    fetch_get_user_result, 
...
#[post("/users/create-batch")]
pub async fn post_users_route(body: String) -> impl Responder {
    let users_request_result: Result<Vec<UserCreateRequest>, Error> = serde_json::from_str(&body);
    let valid_request: bool = match &users_request_result {
        Ok(_) => true,
        Err(_) => false
    };
    ...
    let users_request = users_request_result
        .expect(&[EXCEPT_DEFAULT_MESSAGE, "user_request"].concat());

    fecth_create_users_result(users_request)
}
